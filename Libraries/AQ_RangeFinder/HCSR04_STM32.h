...
#define TRIGGER_CLOCK_SPEED 1000000	// in Hz
...
void inititalizeRangeFinders() {
  ...
  gpio_set_mode(PIN_MAP[ECHO_PIN].gpio_device,
		PIN_MAP[ECHO_PIN].gpio_bit, GPIO_AF_INPUT_PD);
  ...
  *mr |= 0x61 << (8 * (aTimerChannel & 1));	// 0x61 -> 6=filter, 1=inputs 1,2,3,4
  ...
  timer_attach_interrupt(timer_sonar, PIN_MAP[ECHO_PIN].timer_channel,
			 echo_change);

  pinMode(TRIGGER_PIN, PWM);
  timer_generate_update(timer_sonar);
  timer_set_compare(timer_sonar, PIN_MAP[TRIGGER_PIN].timer_channel,
		    TRIGGER_COMPARATOR);
}
